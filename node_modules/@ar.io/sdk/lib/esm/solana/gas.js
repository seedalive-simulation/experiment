...
import { getDelegationSize, getObserverLookupSize, getRedelegationRecordSize, getWithdrawalCounterSize, getWithdrawalSize, } from '@ar.io/solana-contracts/gar';
/**
 ...
 */
export function spawnAntAccountBytes(nameLength) {
 ...
 */
export function getGarWorkflowGasProfile({ workflow, needsDelegation = false, needsWithdrawalCounter = false, needsRedelegationRecord = false, instant = false, }) {
    switch (workflow) {
        case 'join-network':
            return {
                transactionCount: 1,
 ...
 */
export function getIntentGasProfile({ intent, name = '', needsAclBootstrap = false, needsPrimaryNameAccount = false, }) {
 ...
 */
export async function estimateRentLamports(rpc, accountBytes, reallocBytes = 0) {
    if (accountBytes.length === 0 && reallocBytes === 0)
        return 0;
    const totalBytes = accountBytes.reduce((sum, b) => sum + b, 0) +
    ...
    try {
        ...
        return ((totalBytes + RENT_ACCOUNT_OVERHEAD_BYTES) *
