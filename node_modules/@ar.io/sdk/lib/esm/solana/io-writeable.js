 ...
 *      surface for them.
 */
import { AccountRole, address, appendTransactionMessageInstructions, compileTransaction, createTransactionMessage, fetchEncodedAccount, fetchEncodedAccounts, getAddressDecoder, getBase64EncodedWireTransaction, pipe, setTransactionMessageFeePayerSigner, setTransactionMessageLifetimeUsingBlockhash, } from '@solana/kit';
 ...
 */
function altEligibleAddresses(instructions, alwaysInline) {
    const inline = new Set(alwaysInline);
    ...
    for (const ix of instructions) {
        for (const acc of ix.accounts ?? []) {
            if (!inline.has(acc.address))
                eligible.add(acc.address);
        }
    }
    return [...eligible];
}
...
export class SolanaARIOWriteable extends SolanaARIOReadable {
    ...
    async sendTransaction(instructions, computeUnitLimit = DEFAULT_COMPUTE_UNIT_LIMIT, extraSigners = []) {
    ...
    async buyRecord(params, _options) {
        ...
        if (params.processId === undefined) {
            ...
            if (params.antState !== undefined) {
                this.logger.warn('[buyRecord] antState is ignored when buying to an existing ANT ' +
        ...
        if (mintSigner !== undefined) {
            ...
            if (inlineSize <= MAX_TX_SIZE_BYTES) {
                sig = await this.sendTransaction(spawnAndBuyIxs, SPAWN_AND_BUY_COMPUTE_UNIT_LIMIT, [mintSigner]);
            }
            else {
            ...
            return { id: sig, result: { processId: antPubkey } };
        }
     ...
     */
    async _bootstrapSpawnedAntAcl(asset, name) {
        ...
        try {
            ...
            this.logger.warn(`[buyRecord] spawned ANT ${asset} for "${name}" but ACL owner/controller ` +
                `bootstrap failed; reconcile via the sync-ACL API. Purchase + mint ` +
                `already confirmed.`, err);
        }
        ...
        try {
            ...
            this.logger.warn(`[buyRecord] spawned ANT ${asset} for "${name}": ACL recorded, but ` +
                `sync_attributes failed; ANT record traits will reconcile on a later ` +
                `syncAttributes() call.`, err);
        }
    }
    /**
    ...
    async releaseVault(params, _options) {
        ...
        const createOwnerAtaIx = buildCreateAtaIdempotentIx(this.signer.address, ownerATA, this.signer.address, mint);
     ...
     */
    async adminSetRewardRatios(params, _options) {
        const [epochSettings] = await getEpochSettingsPDA(this.garProgram);
            ...
            try {
                ...
                if (old && old.rewardsDistributed === 1) {
                    if (old.observationsSubmitted > old.observationsClosed) {
                        ...
                        const observers = await this.filterLiveObservations(closeTarget, candidates);
     ...
     */
    async maybeArnsLifecycleStep(opts, now, flags) {
        if (flags.toReturned) {
     ...
     */
    async maybePruneToReturnedStep(opts, now) {
        const scanInterval = opts.pruneScanIntervalMs ?? 60_000;
        const wallNow = Date.now();
        if (wallNow - this.lastPruneToReturnedScanMs < scanInterval)
            return null;
     ...
     */
    async maybePruneExpiredNamesStep(opts, now) {
        const scanInterval = opts.pruneScanIntervalMs ?? 60_000;
        const wallNow = Date.now();
        if (wallNow - this.lastPruneExpiredScanMs < scanInterval)
            return null;
     ...
     */
    async filterLiveObservations(epochIndex, observers) {
        if (observers.length === 0)
            return [];
        const pdas = await Promise.all(observers.map(async (obs) => {
            const [pda] = await getObservationPDA(epochIndex, address(obs), this.garProgram);
            return pda;
        }));
