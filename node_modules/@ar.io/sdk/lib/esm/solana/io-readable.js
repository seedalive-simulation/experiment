 ...
 */
import { ARNS_RECORD_DISCRIMINATOR, DEMAND_FACTOR_DOWN_ADJUSTMENT, DEMAND_FACTOR_MIN, DEMAND_FACTOR_SCALE, DEMAND_FACTOR_UP_ADJUSTMENT, MAX_PERIODS_AT_MIN_DEMAND_FACTOR, MOVING_AVG_PERIOD_COUNT, PERIOD_LENGTH_SECONDS, RESERVED_NAME_DISCRIMINATOR, RETURNED_NAME_DISCRIMINATOR, getArnsConfigDecoder, getArnsRecordDecoder, getReservedNameDecoder, getReturnedNameDecoder, } from '@ar.io/solana-contracts/arns';
...
import { GAR_COMPUTE_UNIT_LIMIT, estimateRentLamports, getGarWorkflowGasProfile, getIntentGasProfile, } from './gas.js';
import { TOKEN_PROGRAM_ADDRESS } from './instruction.js';
import { getAclConfigPDA, getArioConfigPDA, getArnsRecordPDA, getArnsRecordPDAFromHash, getArnsSettingsPDA, getDelegationPDA, getDemandFactorPDA, getEpochPDA, getEpochSettingsPDA, getGarSettingsPDA, getGatewayPDA, getGatewayRegistryPDA, getObserverLookupPDA, getPrimaryNamePDA, getPrimaryNameRequestPDA, getRedelegationRecordPDA, getReservedNamePDA, getReturnedNamePDA, getVaultPDA, getWithdrawalCounterPDA, getWithdrawalPDA, } from './pda.js';
import { withRetry } from './retry.js';
import { estimateGasFee, estimateQuotePriorityFeeMicroLamports, } from './send.js';
...
function matchesFilters(item, filters) {
    ...
    for (const [key, expected] of Object.entries(filters)) {
        ...
        if (!accepted.some((e) => valuesEqual(actual, e)))
            return false;
    }
    return true;
}
...
export function paginate(items, params) {
    ...
    const filtered = params?.filters
        ? items.filter((item) => matchesFilters(item, params.filters))
        : items;
    ...
    if (params?.sortBy !== undefined) {
        ...
        ordered = [...filtered].sort((a, b) => {
            ...
            if (cmp === 1 && valueAtPath(a, key) == null)
                return 1;
            if (cmp === -1 && valueAtPath(b, key) == null)
                return -1;
...
export class SolanaARIOReadable {
     ...
     */
    async getClusterUnixTimestampSeconds() {
        const slot = await withRetry(() => this.rpc.getSlot({ commitment: this.commitment }).send());
        const blockTime = await withRetry(() => this.rpc.getBlockTime(slot).send());
        if (blockTime == null) {
            throw new Error(`Cluster block time unavailable for slot ${slot}; cannot match on-chain Clock::get().unix_timestamp`);
        }
    ...
    async getGatewayAccumulators(operatorAddresses) {
        ...
        for (const group of chunk(unique, 100)) {
            const pdas = await Promise.all(group.map(async (op) => (await getGatewayPDA(address(op), this.garProgram))[0]));
    ...
    async getTokenSupply() {
        ...
        // reserve).
        const derivedCirculating = total -
            config.lockedSupply -
            staked -
            delegated -
            withdrawn -
            protocolBalance;
        const circulating = derivedCirculating >= 0 ? derivedCirculating : config.circulatingSupply;
        return {
            total,
            circulating,
    ...
    async getGasEstimate(opts = {}) {
        ...
        if (opts.intent === 'Buy-Name' || opts.intent === 'Buy-Record') {
            ...
            if (opts.fromAddress) {
                try {
                    const [aclPda] = await getAclConfigPDA(address(opts.fromAddress), this.antProgram);
            ...
            if (opts.fromAddress) {
                try {
                    const [pnPda] = await getPrimaryNamePDA(address(opts.fromAddress), this.coreProgram);
    ...
    async getGarGasEstimate(opts) {
        ...
        const isDelegationWorkflow = opts.workflow === 'delegate-stake' ||
            opts.workflow === 'redelegate-stake';
        if (isDelegationWorkflow && opts.fromAddress && opts.gatewayAddress) {
            try {
                const [delegationPda] = await getDelegationPDA(address(opts.gatewayAddress), address(opts.fromAddress), this.garProgram);
        ...
        const isWithdrawalCreatingWorkflow = opts.workflow === 'leave-network' ||
        ...
        if (isWithdrawalCreatingWorkflow && opts.fromAddress) {
            try {
                const [counterPda] = await getWithdrawalCounterPDA(address(opts.fromAddress), this.garProgram);
        ...
        if (opts.workflow === 'redelegate-stake' && opts.fromAddress) {
            try {
                const [recordPda] = await getRedelegationRecordPDA(address(opts.fromAddress), this.garProgram);
        ...
        const isWithdrawalClosingWorkflow = opts.workflow === 'claim-withdrawal' ||
            opts.workflow === 'cancel-withdrawal' ||
            opts.workflow === 'instant-withdrawal';
        if (isWithdrawalClosingWorkflow &&
            opts.fromAddress &&
            opts.vaultId !== undefined) {
            try {
                const [withdrawalPda] = await getWithdrawalPDA(address(opts.fromAddress), BigInt(opts.vaultId), this.garProgram);
        ...
        let rentLamports = this._rentCache.get(rentKey);
        if (rentLamports === undefined) {
            rentLamports = await estimateRentLamports(this.rpc, profile.accountBytes, reallocBytes);
            this._rentCache.set(rentKey, rentLamports);
        }
    ...
    async getTokenCost(params) {
        ...
        const elapsedSincePeriodZero = clusterNowSeconds - df.periodZeroStartTimestamp;
        const periodForNow = elapsedSincePeriodZero < 0
            ? 1
            : Math.floor(elapsedSincePeriodZero / PERIOD_LENGTH_SECONDS) + 1;
        if (periodForNow > df.currentPeriod) {
            this.logger.debug('[pricing] demand factor period has rolled but stored factor is stale; ' +
    ...
    async getDemandFactorSettings() {
        ...
        return {
            ...
            demandFactorUpAdjustmentRate: Number(((DEMAND_FACTOR_UP_ADJUSTMENT - scale) * 1000n) / scale),
            demandFactorDownAdjustmentRate: Number(((scale - DEMAND_FACTOR_DOWN_ADJUSTMENT) * 1000n) / scale),
     ...
     */
    async getPruneableToReturnedRecords(now) {
        const [arnsConfigPda] = await getArnsSettingsPDA(this.arnsProgram);
        const cfgAccount = await this.getCachedAccount(arnsConfigPda);
        if (!cfgAccount.exists)
            return [];
     ...
     */
    async getFinalizableGoneGateways(now) {
        ...
        for (const { pubkey, data } of accounts) {
            try {
                ...
                if (g.status !== GatewayStatus.Leaving)
                    continue;
                if (g.leaveTimestamp.__option !== 'Some')
                    continue;
                // Live delegations must be cranked out first (separate GC path), else
                // finalize_gone reverts.
                if (g.totalDelegatedStake !== 0n)
                    continue;
                const epochDuration = Math.max(Number(g.leaveEpochDuration), currentEpochDuration);
                const eligibleAt = Number(g.leaveTimestamp.value) +
    ...
    async resolveArNSName({ name, }) {
        ...
        if (antRecord === undefined) {
            throw new Error(`ArNS name "${name}" has no "${undernameKey}" record on ANT ${record.processId}`);
        }
        return {
