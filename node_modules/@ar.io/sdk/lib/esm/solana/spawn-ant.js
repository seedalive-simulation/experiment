...
import { estimateComputeUnitLimit, estimatePriorityFeeMicroLamports, estimateQuotePriorityFeeMicroLamports, } from './send.js';
 ...
 */
export function validateSpawnAntState(state) {
    if (!state)
        return;
 ...
 */
export function buildCreateAntInstruction({ mint, authority, payer, updateAuthority, name, uri, attributes = [], }) {
...
export async function spawnSolanaANT(params) {
    ...
    const { instructions: [createIx, initIx], mintSigner, mint, } = await buildSpawnAntInstructions({
        signer,
        state,
        antProgramId,
    ...
    const buildMessage = (units, blockhash) => pipe(createTransactionMessage({ version: 0 }), (tx) => setTransactionMessageFeePayerSigner(signer, tx), (tx) => setTransactionMessageLifetimeUsingBlockhash(blockhash, tx), (tx) => appendTransactionMessageInstructions([
