...
import { deserializeAntConfig, deserializeAntControllers, } from './deserialize.js';
...
export class SolanaANTWriteable extends SolanaANTReadable {
    ...
    async syncAcl(_options) {
        ...
        if (nftOwner !== caller) {
            throw new Error(`Cannot sync ACL: ${caller} is not the current owner of ANT ` +
        ...
        const previousOwner = configAccount
            ? deserializeAntConfig(configAccount.data).owner
            : null;
        const [controllersPda] = await getAntControllersPDA(this.mint, this.antProgram);
        const controllersAccount = await getAccountInfoLegacy(this.rpc, controllersPda, this.commitment);
        const exControllers = controllersAccount
        ...
        ixs.push(await getReconcileInstructionAsync({ asset: this.mint, caller: this.signer }, { programAddress: this.antProgram }));
        // 2. record the caller's owner ACL entry (bootstrap config/page if needed).
        const dest = await this.registry.resolveDestinationAclAccounts({
            user: caller,
        });
        ixs.push(...dest.prepIxs);
        ixs.push(await this.registry.buildRecordIx({
            user: caller,
            asset: this.mint,
            role: 'owner',
            pageIdx: dest.pageIdx,
        ...
        if (previousOwner && previousOwner !== caller) {
            ...
            if (source) {
                ixs.push(await this.registry.buildRemoveIx({
                    user: address(previousOwner),
                    asset: this.mint,
                    role: 'owner',
        ...
        ixs.push(...(await this.registry.bulkRemoveControllerEntries({
            asset: this.mint,
            controllers: exControllers,
        })));
        const sig = await this.sendTransaction(ixs);
