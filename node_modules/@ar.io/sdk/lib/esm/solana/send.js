...
import { addSignersToTransactionMessage, address, appendTransactionMessageInstructions, compileTransaction, compressTransactionMessageUsingAddressLookupTables, createTransactionMessage, getAddressDecoder, getBase64EncodedWireTransaction, getSignatureFromTransaction, isTransactionModifyingSigner, partiallySignTransaction, pipe, sendAndConfirmTransactionFactory, setTransactionMessageFeePayerSigner, setTransactionMessageLifetimeUsingBlockhash, signTransactionMessageWithSigners, } from '@solana/kit';
/**
...
function clampedFeePercentile(recent, percentile) {
    const fees = recent
    ...
    const value = fees[Math.min(fees.length - 1, Math.floor(fees.length * percentile))];
 ...
 */
const WALLET_FEE_PRIOR_MICRO_LAMPORTS = 500000n;
/**
 ...
 */
export async function estimateWalletPriorityFeeMicroLamports(rpc) {
    const samples = await Promise.all(MARKET_FEE_REFERENCE_ACCOUNTS.map(async (account) => {
        try {
 ...
 */
export async function estimateQuotePriorityFeeMicroLamports(rpc) {
 ...
 */
export async function estimateComputeUnitLimit(rpc, message, fallback) {
    try {
        ...
        if (sim.value.err != null || consumed == null)
            return fallback;
        const sized = Math.ceil(Number(consumed) * COMPUTE_UNIT_LIMIT_BUFFER);
        return Math.min(Math.max(sized, MIN_COMPUTE_UNIT_LIMIT), Math.min(fallback, MAX_COMPUTE_UNIT_LIMIT));
    }
    catch {
        return fallback;
    }
 ...
 */
export async function estimateGasFee(rpc, { computeUnitLimit = DEFAULT_COMPUTE_UNIT_LIMIT, signatureCount = 1, transactionCount = 1, rentLamports = 0, rentReclaimedLamports = 0, priorityFeeMicroLamports, } = {}) {
    const microLamports = priorityFeeMicroLamports !== undefined
    ...
    const priorityFeeLamports = transactionCount *
        Number((BigInt(computeUnitLimit) * microLamports + 999999n) / 1000000n);
 ...
 */
export async function sendAndConfirm({ rpc, rpcSubscriptions, signer, instructions, commitment = 'confirmed', computeUnitLimit = DEFAULT_COMPUTE_UNIT_LIMIT, autoComputeUnitLimit = true, priorityFeeMicroLamports = 'auto', addressLookupTables, extraSigners = [], }) {
    ...
    const buildMessage = (units, blockhash) => {
        const baseMessage = pipe(createTransactionMessage({ version: 0 }), (tx) => setTransactionMessageFeePayerSigner(signer, tx), (tx) => setTransactionMessageLifetimeUsingBlockhash(blockhash, tx), (tx) => appendTransactionMessageInstructions([
    ...
    const shouldAutoSize = autoComputeUnitLimit && !isTransactionModifyingSigner(signer);
    const units = shouldAutoSize
        ? await estimateComputeUnitLimit(rpc, buildMessage(computeUnitLimit, latestBlockhash), computeUnitLimit)
        : computeUnitLimit;
    ...
    const messageWithSigners = extraSigners.length > 0
    ...
    if (extraSigners.length > 0 && isTransactionModifyingSigner(signer)) {
        ...
        const preSigned = await partiallySignTransaction(extraSigners.map((s) => s.keyPair), compiled);
        [signedTx] = await signer.modifyAndSignTransactions([preSigned]);
    }
    else {
 ...
 * needed.
 */
export function estimateCompiledTxSize({ signer, instructions, extraSigners = [], computeUnitLimit = DEFAULT_COMPUTE_UNIT_LIMIT, }) {
    const message = pipe(createTransactionMessage({ version: 0 }), (tx) => setTransactionMessageFeePayerSigner(signer, tx), (tx) => setTransactionMessageLifetimeUsingBlockhash({
        blockhash: '11111111111111111111111111111111',
        lastValidBlockHeight: 0n,
    }, tx), (tx) => appendTransactionMessageInstructions([
        getSetComputeUnitLimitInstruction({ units: computeUnitLimit }),
        getSetComputeUnitPriceInstruction({ microLamports: 1n }),
        ...instructions,
    ], tx), (tx) => extraSigners.length > 0
 ...
 * `signer` alone.
 */
export async function sendWithEphemeralLookupTable({ rpc, rpcSubscriptions, signer, instructions, lookupAddresses, commitment = 'confirmed', computeUnitLimit = 1_000_000, extraSigners = [], }) {
 ...
 */
async function waitForLookupTableActive(rpc, table, expectedCount, maxWaitMs = 45_000) {
    const META = 56;
    ...
    while (Date.now() - start < maxWaitMs) {
        ...
        await new Promise((r) => setTimeout(r, Math.min(800, Math.max(0, maxWaitMs - (Date.now() - start)))));
    }
    throw new Error(`lookup table ${table} not finalized with >=${expectedCount} addresses within ${maxWaitMs}ms`);
}
/**
