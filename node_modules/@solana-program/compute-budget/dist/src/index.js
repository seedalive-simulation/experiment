...
function getRequestHeapFrameInstructionDataCodec() {
  return kit.combineCodec(getRequestHeapFrameInstructionDataEncoder(), getRequestHeapFrameInstructionDataDecoder());
}
...
function getRequestUnitsInstructionDataCodec() {
  return kit.combineCodec(getRequestUnitsInstructionDataEncoder(), getRequestUnitsInstructionDataDecoder());
}
...
function getSetComputeUnitLimitInstructionDataCodec() {
  return kit.combineCodec(getSetComputeUnitLimitInstructionDataEncoder(), getSetComputeUnitLimitInstructionDataDecoder());
}
...
function getSetComputeUnitPriceInstructionDataCodec() {
  return kit.combineCodec(getSetComputeUnitPriceInstructionDataEncoder(), getSetComputeUnitPriceInstructionDataDecoder());
}
...
function getSetLoadedAccountsDataSizeLimitDiscriminatorBytes() {
  return kit.getU8Encoder().encode(SET_LOADED_ACCOUNTS_DATA_SIZE_LIMIT_DISCRIMINATOR);
}
...
function findSetComputeUnitLimitInstructionIndexAndUnits(transactionMessage) {
  ...
  const units = kit.getU32Decoder().decode(transactionMessage.instructions[index].data, 1);
  return { index, units };
}
...
async function simulateTransactionAndGetConsumedUnits({
  ...
  try {
    ...
    if (kit.isSolanaError(e, kit.SOLANA_ERROR__TRANSACTION__FAILED_WHEN_SIMULATING_TO_ESTIMATE_COMPUTE_LIMIT)) throw e;
    throw new kit.SolanaError(kit.SOLANA_ERROR__TRANSACTION__FAILED_TO_ESTIMATE_COMPUTE_LIMIT, { cause: e });
  }
}
...
exports.parseComputeBudgetInstruction = parseComputeBudgetInstruction;
