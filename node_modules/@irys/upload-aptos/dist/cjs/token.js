...
class AptosConfig extends base_1.BaseNodeToken {
    constructor(config) {
        ...
        this.aptosConfig = new ts_sdk_1.AptosConfig({
            network: this.providerUrl,
            ...config?.opts?.aptosSdkConfig,
        });
    }
    async getProvider() {
        return (this.providerInstance ??= new ts_sdk_1.Aptos(this.aptosConfig));
    }
    async getTx(txId) {
    ...
    getSigner() {
        ...
        if (this.signingFn) {
            const signer = new bundles_1.AptosSigner('', '0x' + this.getPublicKey().toString('hex'));
            signer.sign = this.signingFn; // override signer fn
            return (this.signerInstance = signer);
        }
        else {
            return (this.signerInstance = new bundles_1.AptosSigner(
            // @ts-expect-error private field use
            this.accountInstance.privateKey.toString(), this.accountInstance.publicKey.toString()));
        }
    }
    async verify(pub, data, signature) {
        return await bundles_1.AptosSigner.verify(pub, data, signature);
    }
    async getCurrentHeight() {
        return new bignumber_js_1.default((await (await this.getProvider()).getLedgerInfo()).block_height);
    }
    async getFee(amount, to) {
        ...
        const [simulationResult] = await (0, async_retry_1.default)(async (_) => {
            const { data } = await (0, ts_sdk_1.postAptosFullNode)({
    ...
    async sendTx(data) {
        ...
        const { data: postData } = await (0, ts_sdk_1.postAptosFullNode)({
            aptosConfig: this.aptosConfig,
            body: data.tx,
            path: 'transactions',
