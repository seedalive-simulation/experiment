...
const upload_1 = require("@irys/upload/upload");
const token_1 = require("./token");
const base_1 = tslib_1.__importDefault(require("@irys/upload/base"));
...
exports.program
    ...
    try {
        ...
        if (confirmed) {
            const res = await irys.withdrawBalance(amount == "all" ? "all" : new bignumber_js_1.default(amount));
...
exports.program
    ...
    try {
        ...
        if (confirmed) {
            const tx = await irys.fund(new bignumber_js_1.default(amount), options.multiplier);
...
async function init(opts, operation) {
    ...
    if (!['balance', 'price'].includes(operation)) {
        ...
        if (opts.wallet === 'default') {
            ...
            if (opts.token === 'arweave' && (await (0, upload_1.checkPath)('./wallet.json'))) {
    ...
    try {
        ...
        // });
        // await irys.ready();
        const token = (0, token_1.getToken)(opts.token.toLowerCase()); /* new getToken(opts.token.toLowerCase())() */
        wallet = wallet ?? '';
        const config = {
            providerUrl: opts.providerUrl,
            contractAddress: opts.contractAddress,
